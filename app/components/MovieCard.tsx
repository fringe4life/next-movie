...
export default function MovieCard({toggleMovies, movie:{imdbID, Title, Poster, Plot, imdbRating, Genre, Runtime}}: MovieCardProps){
    ...
    return (
        <section className="mt-4 flex xs:gap-4 gap-2">
            <Image className="h-37 w-25 rounded-xs " src={Poster} width={100} height={148} alt={Title} />
            <section className="xs:my-[0.625rem]">
                <header className="flex items-center">
                    <h2 className="font-medium whitespace-wrap shrink text-black text-lg max-w-[15ch] xs:max-w-[30ch] text-ellipses line-clamp-1">{titleToUse}</h2>
                ...
                <p className="text-sm/5 max-w-[30ch] text-gray-500  xs:line-clamp-3 text-ellipses">{Plot}</p>
